...
public class Main {
        ...
        try {
            ...
            for (int rowNum = 1; rowNum <= xssfSheet.getLastRowNum(); rowNum++) {
                ...
                if (xssfRow != null) {
                    ...
                    XSSFCell classNameXss = xssfRow.getCell(0);
                    XSSFCell unitNameXss = xssfRow.getCell(1);
                    ...
                    classItem.setUnitName(unitNameXss.getStringCellValue());
                    ...
                    String tempKey = classItem.getClassName() + "," + classItem.getUnitName();
                    if (!classItemMap.containsKey(tempKey)) {
                        classItemMap.put(tempKey, new ArrayList<ClassItem>());
                        keyNum++;
                        //System.out.println("增加了第 "+keyNum+"个课程："+tempKey);
                    } else {
                        //System.out.println(tempKey+" 重复了");
                    }
                    classItemMap.get(tempKey).add(classItem);
                }
            }
            ...
            for (String classKey : classItemMap.keySet()) {
                //System.out.println(classKey + "--\t文件数：" + classItemMap.get(classKey).size());
                //一个key就是一个excel文件，key作为文件名
                //key对应的list作为文件中的数据
                //list数据中 只处理 电子教材、教学视频 对应的文件 其他过滤掉
                List<ClassItem> list = classItemMap.get(classKey);
    ...
    private static void writer(String fileName,String fileType,List<ClassItem> list){
